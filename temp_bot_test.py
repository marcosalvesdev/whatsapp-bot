...
from mixins import WhatsAppBotAuxTools
        ...
        while True:
            for contact_xpath in contacts_xpath:
                try:
                    ...
                    #     contact.click()
                    #     count_first_cash -= 1
                    if contact_name == "Andressa":
                        contact_notifications = self.get_notifications(contact=contact, contact_xpath=contact_xpath)
                        if contact_notifications:
                            contact.click()
                        item, is_first_message_cached = self.check_last_contact_choice(contact_name)
                        if contact_notifications and is_first_message_cached:
                            self.wellcome_message()
                            continue
                        if item:
                            if not contact.is_selected():
                                contact.click()
                            # TODO: Trabalhar agora na melhor forma de infomar para o cliente o que ele selecionou
